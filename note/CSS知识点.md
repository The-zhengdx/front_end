...
## 布局
### 1. 定位(position)
1. 静态定位，每个元素默认属性，将元素放在文档布局流的默认位置
2. 相对定位，允许元素相对于正常文档流中的位置做移动
3. 绝对定位，将元素完全从正常文档流中移出，类似单独放在一个图层中，相对于最近被定位的祖先元素，如果没有，则相对于页面根元素(html)
4. 固定定位，相对于浏览器视口固定
5. 粘性定位，是一种新的定位方式，它会让元素先保持和position: static一样的定位，当它的相对视口位置(offset from the viewport)达到某一个预设值时，他就会像position: fixed一样定位。